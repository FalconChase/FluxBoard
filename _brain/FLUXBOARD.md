...
| FBT021 | — | Port dots now colored by actual live role (`FloorLayout.getAnchorRole`): red = booked as a connection's outgoing/"from" end, green = incoming/"to" end, grey = unbooked — reads straight off existing edge/sketch bookings, no new state. Wire/sketch drag previews always show a red dot at the origin + green at the head; a sketch's loose end also visually snaps onto the nearest point of any OTHER path/sketch (alignment only, no attachment — junctions deferred). Node placement and drag-to-move both hard-block against landing one node on top of another (`FloorLayout.wouldOverlap`, 2×NODE_RADIUS) — path-vs-node/path overlap stays deferred until junctions/elevators exist. | SES030 |
