...
| ID | PRIORITY | ITEM | BLOCKED BY |
|----|----------|------|------------|
| —  | —        | Falcon to verify the OBJECTS registry, silent-rejection feedback, multi-select/duplicate/pan, AND the quick-select flyout end-to-end locally (`npm run tauri:dev`), then decide: path-vs-node/path overlap + junctions (deferred per Falcon), INSERT tab's icon/label overlay, FBP012 (object-type-to-path-style taxonomy), FBP013 (LAYERS z-axis), Milestone 6 (isometric, stretch), or something else. | — |
...
---
# Lines: 105 / 120 — Budget remaining: 15
